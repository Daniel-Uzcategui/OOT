...
static ItemData itemData[88] = {
    ...
    {CREATE_SPRITE_24(dgQuestIconMagicJarSmallTex, 97),    ITEM_INVALID_1,     UPG_IC_POS(2, 0), SIZE_NORMAL},
    {CREATE_SPRITE_24(dgQuestIconMagicJarBigTex, 97),      ITEM_INVALID_2,     UPG_IC_POS(2, 0), SIZE_NORMAL},
    ...
    {CREATE_SPRITE_24(dgQuestIconHeartContainerTex, 101),  ITEM_INVALID_3,   {0x05, -0x04},    SIZE_COUNTER},
};
...
u8 ShouldRenderItem(s16 fileIndex, u8 item) {
    ...
    //magic
    if (item == ITEM_INVALID_1 && HasItem(fileIndex, ITEM_INVALID_2)) {
        return 0;
    }

    if (item == ITEM_INVALID_2 && !HasItem(fileIndex, ITEM_INVALID_2)) {
        return 0;
    }
    ...
    if (item == ITEM_INVALID_3 && !Save_GetSaveMetaInfo(fileIndex)->isDoubleDefenseAcquired) {
        return 0;
    }
